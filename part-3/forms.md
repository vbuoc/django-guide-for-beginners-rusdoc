...
**GET** is perhaps the most common request type. It’s used to _retrieve_ data from the server. Every time you click on a link or type a URL directly into the browser, you are creating a **GET** request.
...
It can look broken right now but trust me; there’s a lot of things behind it right now. And it’s extremely powerful. For example, if our form had 50 fields, we could render all the fields just by typing `<span class="p">{</span><span class="err">{</span> <span class="w"></span> <span class="err">form.as_p</span> <span class="w"></span> <span class="p">}</span><span class="err">}</span>`.
...
The next form we implement, we can simply use `<span class="p">{</span><span class="err">%</span> <span class="w"></span> <span class="err">include</span> <span class="w"></span> <span class="err">'includes/form.html'</span> <span class="w"></span> <span class="err">%</span><span class="p">}</span>` to render it.
